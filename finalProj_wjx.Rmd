...
The null hypothesis is: There is no association between type of urinary diversion and SRE development. And the alternative hypothesis is there is an association between type of urinary diversion and SRE development. First we use chi-square's test to compute the expected value of the obtained contigency table, we found that none of the expected counts in this table less than 5, so Chi-square Test is appropriate for this situation. Therefore, we use Chi-square Test to test the dependency Type of urinary diversion and SRE development.
```{r  results='hide'}
sreUri <- table(sreBladder2m$type_of_urinary_diversion,
                sreBladder2m$sreStatus)
expected <- chisq.test(sreUri,correct=FALSE)$expected %>% round(3)
expected %>%
  kbl() %>%
...
```{r results='hide'}
result1 <- chisq.test(sreUri, correct=FALSE)
result1
...
sreBladder2m %>%
  select(type_of_urinary_diversion, stage_at_diagnosis, type_of_metastasis_at_diagnosis, sreStatus)%>%
  tbl_summary(label = list(type_of_urinary_diversion ~"Type of urinary diversion",
...
tablesP <- matrix(c(sreAge$p.value,
                    chisq.test(sreUri, correct=FALSE)$p.value,
...
```{r}
interestVar <- c("Age","Uri","Stage", "Meta")
tbl_demo <- tbl %>%
